...
auto parse_level(std::string_view value) noexcept -> Result<Level, ParseError> {
  if (value == "TRACE")
    return Level::trace;
  if (value == "DEBUG")
    return Level::debug;
  ...
  if (value == "FATAL")
    return Level::fatal;
  return unexpected(ParseError{ParseErrorCode::invalid_level, "invalid level"});
}
...
auto parse_integer(std::string_view value) noexcept
...
auto parse_line(std::string_view line) noexcept -> ParseResult {
  ...
  if (!split_fields(line, fields)) {
    return unexpected(ParseError{ParseErrorCode::wrong_field_count,
                                 "expected exactly five fields"});
  }
  if (!valid_timestamp(fields[0])) {
    return unexpected(
        ParseError{ParseErrorCode::invalid_timestamp, "invalid UTC timestamp"});
  }

  const auto level = parse_level(fields[1]);
  if (!level)
    return unexpected(level.error());

  if (!valid_service(fields[2])) {
    return unexpected(
        ParseError{ParseErrorCode::invalid_service, "invalid service name"});
  }

  const auto latency = parse_integer<std::uint32_t>(fields[3]);
  if (!latency) {
    return unexpected(
        ParseError{ParseErrorCode::invalid_latency, "invalid latency"});
  }
