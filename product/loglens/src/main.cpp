...
auto positive_size(std::string_view value) -> std::expected<std::size_t, std::string_view> {
  std::uint64_t parsed{};
  const auto result = std::from_chars(value.data(), value.data() + value.size(), parsed);
  if (value.empty() || result.ec != std::errc{} ||
      result.ptr != value.data() + value.size() || parsed == 0 ||
      parsed > std::numeric_limits<std::size_t>::max()) {
    return std::unexpected("limit must be a positive integer");
  }
  return static_cast<std::size_t>(parsed);
}

auto parse_options(int argc, char** argv)
    -> std::expected<Options, std::string_view> {
  Options options;
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      if (argc != 2) return std::unexpected("--help cannot be combined with other options");
      usage(std::cout);
      std::exit(0);
    }
    if (argument == "--version") {
      if (argc != 2) return std::unexpected("--version cannot be combined with other options");
      std::cout << "loglens " << kVersion << '\n';
      std::exit(0);
    }
    if (argument == "--strict") {
      if (saw_strict) return std::unexpected("duplicate --strict");
      saw_strict = true;
      options.strict = true;
      continue;
    }
    ...
    if (!takes_value || index + 1 >= argc) {
      return std::unexpected("unknown or incomplete argument");
    }
    const std::string_view value = argv[++index];
    if (argument == "--input") {
      if (saw_input) return std::unexpected("duplicate --input");
      saw_input = true;
      options.input = value;
    } else if (argument == "--max-line-bytes") {
      if (saw_max_line) return std::unexpected("duplicate --max-line-bytes");
      saw_max_line = true;
      const auto parsed = positive_size(value);
      if (!parsed) return std::unexpected(parsed.error());
      options.max_line_bytes = *parsed;
    } else {
      if (saw_max_services) return std::unexpected("duplicate --max-services");
  ...
  if (!saw_input || options.input.empty()) return std::unexpected("--input is required");
  return options;
}
...
auto run(int argc, char** argv) -> int {
  ...
  for (;;) {
    ...
    if (result == loglens::ReadLineResult::end) break;
    ++line_number;
    ...
    if (result == loglens::ReadLineResult::too_long) {
      std::cerr << "loglens: line " << line_number << ": exceeds --max-line-bytes\n";
      return 5;
    }
